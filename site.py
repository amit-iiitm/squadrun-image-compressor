...
def readURL(csv_file):
	...
	reader=csv.reader(codecs.open(csv_file, 'rU', 'utf-8'))
	for line in reader:
		for col in line:
			print col
			if checkURL(col):
...
def compress_img():
	...
	#generate a uid for this file used for storing the file
	uid=generate_uid()
	#combine the original file name with uid to save and refrence that file
	file_name=file_obj.filename.split('.')[0]+'_'+uid+'.'+file_obj.filename.split('.')[-1]
	file_obj.save(secure_filename(file_name))
	print file_name
	print type(file_name)
	if file_name.split('.')[-1] in ['JPG','JPEG','PNG','png','jpg','jpeg','gif','GIF','tiff','TIFF','bmp','BMP']:
		#valid image file has been uploaded
		print "valid image file"
		size_red=float(size_red)/100
		qlty=int(qlty)
		print type(size_red)
		response=json.loads(compress_image(file_name,qlty,size_red))
		...
		with open('result_'+uid+'.csv','w') as f:
			writer=csv.writer(f)
			for i in res_list:
				writer.writerow([i])
		return json.dumps({'response':'successfully processed csv file containing image urls','type':"CSV file",'download_link':'http://'+host_port+'/download/result_'+uid+'.csv'})
	else:
...
#http://eeweb.poly.edu/~yao/EL5123/image/lena_color.tiff
...
if __name__ == '__main__':
    app.run(debug=True, host='0.0.0.0',threaded=True,port=5050)
